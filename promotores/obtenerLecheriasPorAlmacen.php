...
try {
    ...
    $sql_lep = "SELECT SURTIMIENTO AS SURT_CAJAS, VENTA_REAL AS VENTA_LITROS, INVENTARIO_FINAL AS FIN_LITROS
    ...
    foreach ($lecherias as &$lech) {
        ...
        if ($inv) {
            ...
            if ($lep) {
                ...
                $lech['dot_recibida_cajas'] = $surtCajas;
                $lech['abasto_cajas']       = $surtCajas;
                $lech['abasto_sobres']      = 0;
                $lech['venta_cajas']        = (int)floor($ventaLitros / $L_X_CAJA);
                $lech['venta_sobres']       = (int)floor(($ventaLitros % $L_X_CAJA) / $L_X_SOBRE);
                $lech['inv_fin_cajas']      = (int)floor($finLitros / $L_X_CAJA);
                $lech['inv_fin_sobres']     = (int)floor(($finLitros % $L_X_CAJA) / $L_X_SOBRE);
                $lech['retiro_cajas']       = 0;
                $lech['retiro_sobres']      = 0;
                $lech['fecha_entrada']      = null;
                $lech['caducidad']          = null;
            } else {
